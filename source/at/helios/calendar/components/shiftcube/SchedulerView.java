...
import at.helios.calendar.components.HolidayBox;
import at.helios.calendar.components.OtherShiftsBox;
import at.helios.calendar.components.ViolationListbox;
...
import at.helios.sheduling.shiftcube.ScheduleGenerationThread;
import at.helios.sheduling.shiftcube.SchedulingThread;
...
{
    private ShiftCube            _oNEFShiftCube;
    private ShiftCube            _oRTWShiftCube;
    private ShiftCube            _oSTBShiftCube;
    ...
    public void onGeneratePlan()
    {
        ...
        if (oCalendarFrom.after(oCalendarTo))
        {
            try
            {
                Messagebox
                    .show(
                        "Von-Datum liegt zeitlich hinter dem Bis-Datum, wählen Sie ein späteres Bis-Datum.",
        ...
        Collection<ShiftCube> coShiftCubes = new ArrayList<ShiftCube>();
        ...
        _oNEFShiftCube = new ShiftCube(oCalendarFrom, oCalendarTo, (Department)Sessions.getCurrent().getAttribute("department"),
            Teams.nef, _oNEFCount.getValue(), _oHolidayHelper, _oShiftplan.getNEFShifts());
        _oRTWShiftCube = new ShiftCube(oCalendarFrom, oCalendarTo, (Department)Sessions.getCurrent().getAttribute("department"),
            Teams.rtw, _oRTWCount.getValue(), _oHolidayHelper, _oShiftplan.getRTWShifts());
        _oSTBShiftCube = new ShiftCube(oCalendarFrom, oCalendarTo, (Department)Sessions.getCurrent().getAttribute("department"),
            Teams.standby, _oSTBCount.getValue(), _oHolidayHelper, _oShiftplan.getSTBShifts());
        
        coShiftCubes.add(_oNEFShiftCube);
        coShiftCubes.add(_oRTWShiftCube);
        coShiftCubes.add(_oSTBShiftCube);
        _oSchedulingThread = new ScheduleGenerationThread(coShiftCubes, oWindow, Executions.getCurrent().getDesktop());
...
//            }
            ...
            fillRow(oRow, oTempCalendar, _oNEFShiftCube, _oNEFCount.getValue());
            fillRow(oRow, oTempCalendar, _oRTWShiftCube, _oRTWCount.getValue());
            fillRow(oRow, oTempCalendar, _oSTBShiftCube, _oSTBCount.getValue());
        ...
        oButton = new Button();
        oButton.setLabel("Update");
        oButton.addForward("onClick", this, "onUpdateShiftboxes");
        oButton.setWidth("150px");
        oButton.setParent(_oHolidayBox.getVbox());
    ...
    public void onUpdateShiftboxes()
    {
        updateShiftBoxes();
    }
    ...
    {
        Collection<ShiftCube> coShiftTrees = new ArrayList<ShiftCube>();

        BusySchedulingWindow oWindow = (BusySchedulingWindow) Executions.createComponents("/busy_scheduling_window.zul", null, null);
        oWindow.setClosable(false);
        oWindow.setVisible(true);
        
        coShiftTrees.add(_oNEFShiftCube);
        coShiftTrees.add(_oRTWShiftCube);
        coShiftTrees.add(_oSTBShiftCube);
        ...
        System.out.println("jjiijji_ä_Ü*:");
        System.out.println(_oShiftplan);
    }
    ...
    private void fillRow(Row oRow, GregorianCalendar oCalendarTemp, ShiftCube oShiftCube, int iCount)
    {       
        for (int i = 0; i < iCount; i++)
        {
            ShiftIdentifier oShiftIdentifier = oShiftCube.getShiftIdentifier(oCalendarTemp, Shifts.nightshift);
            ...
            if (oShiftIdentifier != null)
            {
                ...
                ShiftBox oShiftBox = new ShiftBox(oShiftIdentifier, oTeam, oShiftCube, i);
            ...
            oShiftIdentifier = oShiftCube.getShiftIdentifier(oCalendarTemp, Shifts.dayshift);
